...
def get_mood_and_create_time_index(current_indiv, period):
    ...
    return mood_indiv
def get_features_for_individual(current_indiv, feature_names, period):
...
'''
def get_Xy(current_indiv, days_prior, features_all_indivs, period):  
...
for period in ['D', '180T']:
    ...
    for days_prior in [1, 4, 7]:
        results_dict[period][days_prior] = {}
        scores = {}
        predictions = {}
        test_subset = {}
        ...
        for current_indiv in indiv_ids:
            ...
            X, y, X_colnames = get_Xy(current_indiv, days_prior, features_all_indivs,
           period)
        ...
        start = True
        for current_indiv in indiv_ids:
            X, y, _ = get_Xy(current_indiv, days_prior, features_all_indivs,
                             period)
            train_size = np.int(np.round(train_subset_propn * len(y), 0))
            train_subset = np.arange(train_size)
            if start:
        ...
        for current_indiv in indiv_ids:
            ...
            allYs = np.hstack((allYs, np.array(y[tst_idxs])))
            
        results_dict[period][days_prior] = [predictions, predictions_big, allYs]
#%%
import cPickle as cp
#cp.dump(results_dict, open('results_dict.dump', 'wb'))
#%%
ys = np.zeros((0,))
results = []
for curr_period in ['D', '180T']:
    for curr_days_prior in [1,4,7]:
        for big_or_indiv in ['big', 'indiv']:
            yb = np.zeros((0,))
            for current_indiv in indiv_ids:
                _, y, _ = get_Xy(current_indiv, curr_days_prior, features_all_indivs, 
                                 curr_period)
                train_size = np.int(np.round(train_subset_propn * len(y), 0))
                train_subset = np.arange(train_size)
                y_bench = get_benchmark_predictions(y, train_subset)
                yb = np.hstack((yb, y_bench['mean']))
            yy = results_dict[curr_period][curr_days_prior][2]
            scr = np.mean(np.power(yy - yb, 2))
            results.append['bench_mean',  curr_period, curr_days_prior, big_or_indiv, scr]
            if big_or_indiv == 'big':
                bigind = 1
            else:
                bigind = 0
            models = results_dict[curr_period][curr_days_prior][bigind][indiv_ids[0]].keys()
            for model in [m for m in models if m != 'arima']:
                preds = np.zeros((0,))
                for current_indiv in indiv_ids:
                    curr_preds = results_dict[curr_period][curr_days_prior][bigind][current_indiv][model]
                    preds = np.hstack((preds, curr_preds))
                yy = results_dict[curr_period][curr_days_prior][2]
                scr = np.mean(np.power(yy - preds, 2))
                results.append([model, curr_period, curr_days_prior, big_or_indiv, scr])
results = pd.DataFrame(results, columns = ['model', 'period', 'days_prior', 'model_type', 'mse'])
results = results[results['model']!='sgdlm']
results['rmse'] = np.sqrt(results['mse'])
results.sort_values(by=['model', 'period', 'days_prior', 'model_type'])
