...
model = LambdaRank(num_features, 'LambdaRank', lr, train_queries=queries_train,
                   width=[40,20,5], normalise=True)
# Get params from our pointwise model and load into lambdaRank model
nn_params_loaded = cPickle.load(open('nn_params.dump', 'rb'))
lasagne.layers.set_all_param_values(model.output_layer, nn_params_loaded[0])
